...
本地事务：事务仅对单一数据库资源济宁访问控制。

分布式事务：多数据库资源，或者跨服务操作。



CAP:Consistent、Available、Partition



因为使用springboot，可以通过注解开启事务控制，同时spring也支持手动回滚以及设置事务回滚点：
...
#### 9.2.1 本地消息表（异步确保）事务最终一致性

将分布式事务**拆分**本地事务进行处理，通过异步通知消息中间件完成事务的最终一致性：

<img src="./images/事务最终一致性.png" style="zoom:67%;" />
...
考虑服务支付服务验证成功后通知订单服务进行对应操作（修改状态、填写支付信息）

<img src="./images/事务最终一致性01.png" style="zoom:80%;" />

这是一个同步的操作，订单服务异常情况会消耗大量的请求时间，为了提高响应速度，这里引入消息中间件进行异步处理，每次把消息保存在队列中，由消费方进行监听：

<img src="./images/事务最终一致性02.png" style="zoom:80%;" />
...
2. mq消息丢失
3. 订单服务因为网络监听不到
4. 订单服务操作失败，没有办法回滚（可以反向再投递，仍然存在上面三个问题）

也就是都属于网络引入的一致性问题。



这里在使用异步mq的前提下（优点是保障了反应速度），为了保证最终的数据一致性，导入本地的消息表。

<img src="./images/事务最终一致性03.png" style="zoom:80%;" />

如果失败了，作为消费方的订单服务也可以使用mq通知，由生产方支付服务进行回滚操作。
...
同时，如果我们有定时任务，也可以在定时任务来负责发送mq消息。



#### 9.2.2 XA与二阶段提交（2PC）

**有时间看下这个：https://yq.aliyun.com/articles/143003**



两个角色：

- 事务协调者（事务管理器）coordinator：协调多个参与者进行事务投票与提交（回滚）
- 事务参与者（资源管理器）participants：本地事务执行者



通常是XA支持二阶段提交，但实际上XA也支持一阶段提交，不过本地事务只支持一阶段提交，分布式事务支持二阶段提交。
...
- 准备阶段：协调者向参与者发送准备消息，参与者如果直接返回失败，要么在本地执行事务。
- 提交阶段：先进行预提交，协调者根据多个参与者的指令再次发出提交或回滚指令。

简单说就是协调器进行多个数据源的预提交确认，从而保证整个事务的准确性。

2PC中2指两个阶段、P准备阶段（Prepare Phase）、C提交阶段（Commit Phase）



**这里关于这个与后面seata的讲得不错的可以看：JAVA 强一致性 2PC两阶段提交介绍以及Seata AT模式实现：https://blog.csdn.net/qq_36872046/article/details/106716125**



**2PC会导致一直等待事务管理器做决定，事务管理器也会等待两个参与者的响应，性能较差。**



#### 9.2.3 代码补偿（补偿事务TCC）

TCC：与2PC类似，但其实现是使用代码来实现的

- Try：业务上预留资源
- Confirm：业务执行
- Cancel：回滚

实现比较复杂，需要在业务上保证数据操作的幂等。



#### 9.2.4 MQ事务消息-最终一致

参考：https://juejin.im/post/6844904106532962311

RocketMq支持事务消息



可以到RocketMq文档里面看对应的分布式事务解决方案。



### 9.3 seata

2019年开源阿里项目Fescar，后面更名为Seata。
...
registry.conf:注册配置，这里使用file类型的注册，也就是后面file.conf这个文件
...
`vgroup_mapping.my_test_tx_group = "default"`对应每个微服务的配置：`spring.cloud.alibaba.seata.tx-service-group=my_test_tx_group `
...
这里注意windows下启动需要在seata目录下新建logs目录，否则会报：

[0.008s][error ][logging] Error opening log file 'C:\seata\bin\\../logs/seata_gc.log': No such file or directory
