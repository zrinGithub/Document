...
这里的lgR()就是用2为底求对数，这样计算出来能够使用多少bit来代表一个索引：
...
这里为了保存索引位置和字符顺序，使用了两个数组alphabet和inverse来维护里面的数据和位置的关系。

- alphabet 维护	索引->字符的关系
- inverse 维护       字符->索引的

这样每次需要通过字符查询就使用inverse数组，如contains、toIndex、
同样使用索引查询使用alphabet查询，如toChar、
...
在后面的算法中，我们会看到字母表可以缩小处理的数据范围，就像计数法排序（键索引计数法）一样，我们分的组越少，处理的速度越快。同时，因为我们同时维护了索引和字符可以快速检索（空间换取时间）。
...
#### 高位优先（MSD most-significant-digit-first）的字符串排序
...
因此我们之前使用键索引计数法里面的count[]对应的索引可以做对应的修改，如果到达结尾，字符串索引返回-1，我们把得到的值+1作为count[]的索引，也就是说如果字符共有R个，那么现在的count[]应该初始化为new int[R+2](包括处理已到结尾的字符串以及键索引计数本来就需要的多一位)
...
最后循环里面的代码：`sort(a, lo + count[r], lo + count[r+1] - 1, d+1, aux);`比较巧妙，因为此时已经进行了排序，所以每个位置的计数实际上是该字符最后一个元素的索引+1，除了已经到尾部的字符串之外，我们需要对每一个段的数据进行针对下一个字母的排序。



在数据已经足够少的情况下，直接使用插入排序：
