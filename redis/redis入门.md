  ...
  - 你有什么解决方案来防止缓存雪崩？
    - 加锁排队
      ...
      - key： 互斥锁解决
        ...
        Redis的SETNX去set一个mutex key， 当操作返回成功时（缓存里面没有这个键），再进行load db的操作并回设缓存； 否则，就重试整个get缓存的方法
    - 数据预热
...
- 你有什么解决方案来防止缓存穿透？
  - 采用布隆过滤器BloomFilter（占用内存少，bit存储性能更高）
